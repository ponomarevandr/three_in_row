...
SceneMainMenu::SceneMainMenu(Application* application): Scene(application) {
	auto button_play = std::make_unique<Button>(this, Graphics::Point(0, 10),
	...
	auto button_about = std::make_unique<Button>(this, Graphics::Point(0, 12),
		Graphics::getScreenWidth() - 1, 0);
	button_about->setText(L"Об игре");
	items.push_back(std::move(button_about));

	auto button_exit = std::make_unique<Button>(this, Graphics::Point(0, 14),
		Graphics::getScreenWidth() - 1, 0);
